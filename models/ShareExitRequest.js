...
const shareExitRequestSchema = new mongoose.Schema({
    ...
    quotaReservationVersion: { type: Number, enum: [1], required: true },
    quotaPeriodKey: { type: String, required: true, match: /^\d{4}-\d{2}$/ },
    quotaReservationState: { type: String, enum: ['reserved', 'consumed', 'released'], required: true },
